...
# --- Organized Admin Menu Handlers ---

async def handle_admin_analytics_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Shows analytics and reporting menu."""
    query = update.callback_query
...
async def handle_admin_products_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Shows product management menu."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id): 
        return await query.answer("Access denied.", show_alert=True)
...
async def handle_admin_locations_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Shows location management menu."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id): 
        return await query.answer("Access denied.", show_alert=True)
...
async def handle_admin_users_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Shows user management menu."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id): 
        return await query.answer("Access denied.", show_alert=True)
...
async def handle_admin_marketing_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Shows marketing and promotions menu."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id): 
        return await query.answer("Access denied.", show_alert=True)
...
async def handle_adm_toggle_discount(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        # FIX: Use boolean toggle instead of integer comparison (PostgreSQL uses BOOLEAN type)
        new_status = not current_status  # True -> False, False -> True
...
async def handle_adm_search_username_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"DB error searching for user '{search_term}': {e}", exc_info=True)
        await send_message_with_retry(context.bot, chat_id, f"❌ Database error during search: {str(e)[:100]}", parse_mode=None)
        return
    finally:
        if conn: 
            conn.close()
