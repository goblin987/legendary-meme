...
async def _get_nowpayments_estimate(target_eur_amount: Decimal, pay_currency_code: str) -> dict:
    ...
    # Import currency validation function
    from utils import is_currency_supported
    
    # Validate currency is supported before making API call
    if not is_currency_supported(pay_currency_code):
        logger.error(f"Currency {pay_currency_code} is not supported by NOWPayments")
...
async def create_nowpayments_payment(
    ...
    # Add a safety margin (5%) to account for price fluctuations and precision issues
    safety_margin = Decimal('1.05')  # 5% safety margin
    safe_minimum_amount = min_amount_api * safety_margin
    
    logger.info(f"NOWPayments minimum amount check with safety margin: {estimated_crypto_amount} {pay_currency_code} vs safe minimum {safe_minimum_amount} {pay_currency_code} (estimated >= safe_minimum: {estimated_crypto_amount >= safe_minimum_amount})")
    
    # Check if crypto amount is below the safe minimum required by API - for BOTH purchases and refills
    if estimated_crypto_amount < safe_minimum_amount:
         logger.warning(f"{'Purchase' if is_purchase else 'Refill'} for user {user_id} ({target_eur_amount} EUR -> {estimated_crypto_amount} {pay_currency_code}) is below the safe minimum {safe_minimum_amount} {pay_currency_code} (API minimum: {min_amount_api}).")
         
         # Convert safe minimum crypto amount back to EUR for user-friendly error message
         try:
             crypto_price_eur = get_crypto_price_eur(pay_currency_code)
             if crypto_price_eur:
                 min_eur_amount = safe_minimum_amount * crypto_price_eur
         ...
         return {
             ...
             'min_amount': f"{safe_minimum_amount:.8f}".rstrip('0').rstrip('.'),
...
async def _finalize_purchase(user_id: int, basket_snapshot: list, discount_code_used: str | None, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # 📊 TRACKING: Log basket contents at start of finalization
    logger.info(f"📊 FINALIZE START: User {user_id} has {len(basket_snapshot)} items in basket")
    for idx, item in enumerate(basket_snapshot):
        logger.info(f"📊 FINALIZE: Item {idx+1}/{len(basket_snapshot)} - P{item.get('product_id', item.get('id', 'UNKNOWN'))} - {item.get('name', 'Unknown')}")
    ...
    try:
        ...
        for item_snapshot in basket_snapshot: # Iterate directly over the rich snapshot
            ...
            # Track referral purchase and VIP level up for the first item (to avoid multiple rewards for same purchase)
            if item_snapshot == basket_snapshot[0]:  # Only for the first item in basket
                try:
                    from referral_system import process_referral_purchase
                    total_purchase_amount = sum(Decimal(str(item['price'])) for item in basket_snapshot)
                    await process_referral_purchase(user_id, total_purchase_amount)
                except Exception as referral_error:
                    logger.error(f"Error processing referral for user {user_id}: {referral_error}")
                
                # Check for VIP level up after purchase
                try:
                    ...
                    if user_result:
                        ...
                        await process_vip_level_up(user_id, new_purchase_count, context.bot)
                except Exception as vip_error:
                    logger.error(f"Error processing VIP level up for user {user_id}: {vip_error}")
    ...
    if db_update_successful:
        ...
        if chat_id:
            try:
                ...
                logger.info(f"📦 DELIVERY TRACKING: Starting delivery for user {user_id} with {len(processed_product_ids)} products: {processed_product_ids}")
...
<i>This one-time setup ensures your privacy and security! 🔒</i>"""
                ...
                if not use_userbot_delivery or fallback_to_bot:
                    ...
                    for prod_id in processed_product_ids:
                        delivery_counter += 1
                        logger.info(f"📦 BOT DELIVERY: Processing product {delivery_counter}/{len(processed_product_ids)} - P{prod_id}")
                        item_details_list = final_pickup_details.get(prod_id)
                        if not item_details_list: 
                            logger.warning(f"📦 BOT DELIVERY: No details for P{prod_id}, skipping")
                            continue
                        ...
                        logger.info(f"📦 BOT DELIVERY: ✅ Completed delivery {delivery_counter}/{len(processed_product_ids)} for P{prod_id}")
